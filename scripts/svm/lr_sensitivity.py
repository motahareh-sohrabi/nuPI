...
if __name__ == "__main__":
    ...
    tags = "('paper_runs', 'svm_lr_sensitivity',)"
    wandb_mode = "online"
    ...
    # LOG_DUAL_LRs = np.linspace(-5, 1, 20, endpoint=True)
    # add some values higher than 1 with same distance as previous ones
    LOG_DUAL_LRs = np.linspace(1, 1.5, 5, endpoint=False)
    DUAL_LRs = 10**LOG_DUAL_LRs

    NU_list = [0]
    KP_list = [0, 1, 5, 10, 50, 70, 100, 200]
    ...
    momentum_values = [-0.1, 0.1, -0.3, 0.3, -0.5, 0.5, -0.7, 0.7, -0.9, 0.9]
    ...
    if not args.draft:
        utils.call_distributed(ALL_COMMANDS, max_tasks=8)

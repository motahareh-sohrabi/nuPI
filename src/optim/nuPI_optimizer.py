    ...
    def step(self, closure=None) -> Optional[float]:
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                step_counter = self.state[p].get("step", 0)
                update_function(
                    param=p,
                    state=self.state[p],
                    lr=group["lr"],
                ...
                self.state[p]["step"] = step_counter + 1
...
def _nupi_zero_init(
    ...
    if use_bias_correction:
        xit_m1_coef /= 1 - ema_nu**step_counter
...
def _nupi_sgd_init(
    ...
    if uses_kp_term:
        if "xi" in state:
            if use_bias_correction:
                bias_corrected_xi = state["xi"] / (1 - ema_nu**step_counter)
            else:
                bias_corrected_xi = state["xi"]

            kp_term_contribution = (1 - ema_nu) * (detached_error - bias_corrected_xi)
